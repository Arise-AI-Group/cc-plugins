...
if command -v python3 &> /dev/null; then
    ...
    [ "$name" = "sop" ] && pass "Plugin name correct: $name" || fail "Plugin name incorrect: $name"
fi
...
if [ -f "tool/extract_sop.py" ]; then
    ./run tool/extract_sop.py --help > /dev/null 2>&1 && pass "extract_sop.py --help works" || echo "  Note: extract_sop.py --help returned non-zero (may be expected)"
fi
if [ -f "tool/transcribe_audio.py" ]; then
    ./run tool/transcribe_audio.py --help > /dev/null 2>&1 && pass "transcribe_audio.py --help works" || echo "  Note: transcribe_audio.py --help returned non-zero (may be expected)"
fi
