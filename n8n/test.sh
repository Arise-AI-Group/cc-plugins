...
if command -v python3 &> /dev/null; then
    ...
    [ "$name" = "n8n" ] && pass "Plugin name correct: $name" || fail "Plugin name incorrect: $name"
fi
...
if [ -f "tool/n8n_api.py" ]; then
    ./run tool/n8n_api.py --help > /dev/null 2>&1 && pass "n8n_api.py --help works" || echo "  Note: n8n_api.py --help returned non-zero (may be expected)"
fi
