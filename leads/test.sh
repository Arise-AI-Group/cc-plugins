...
if command -v python3 &> /dev/null; then
    ...
    [ "$name" = "leads" ] && pass "Plugin name correct: $name" || fail "Plugin name incorrect: $name"
fi
...
if [ -f "tool/verify_leads.py" ]; then
    ./run tool/verify_leads.py --help > /dev/null 2>&1 && pass "verify_leads.py --help works" || echo "  Note: verify_leads.py --help returned non-zero (may be expected)"
fi
if [ -f "tool/update_sheets.py" ]; then
    ./run tool/update_sheets.py --help > /dev/null 2>&1 && pass "update_sheets.py --help works" || echo "  Note: update_sheets.py --help returned non-zero (may be expected)"
fi
if [ -f "tool/scrape_leads.py" ]; then
    ./run tool/scrape_leads.py --help > /dev/null 2>&1 && pass "scrape_leads.py --help works" || echo "  Note: scrape_leads.py --help returned non-zero (may be expected)"
fi
