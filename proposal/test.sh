#!/bin/bash
# Test script for proposal plugin
# Usage: ./test.sh
...
if command -v python3 &> /dev/null; then
    ...
    [ "$name" = "proposal" ] && pass "Plugin name correct: $name" || fail "Plugin name incorrect: $name"
fi
...
if [ -f "tool/generate_proposal.py" ]; then
    ./run tool/generate_proposal.py --help > /dev/null 2>&1 && pass "generate_proposal.py --help works" || echo "  Note: generate_proposal.py --help returned non-zero (may be expected)"
fi
if [ -f "tool/copy_slides_template.py" ]; then
    ./run tool/copy_slides_template.py --help > /dev/null 2>&1 && pass "copy_slides_template.py --help works" || echo "  Note: copy_slides_template.py --help returned non-zero (may be expected)"
fi
