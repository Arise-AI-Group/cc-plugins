#!/bin/bash
# Test script for md-export plugin
# Usage: ./test.sh
...
if command -v python3 &> /dev/null; then
    ...
    [ "$name" = "md-export" ] && pass "Plugin name correct: $name" || fail "Plugin name incorrect: $name"
fi
...
if [ -f "tool/md_to_docx.py" ]; then
    ./run tool/md_to_docx.py --help > /dev/null 2>&1 && pass "md_to_docx.py --help works" || echo "  Note: md_to_docx.py --help returned non-zero (may be expected)"
fi
if [ -f "tool/md_to_gdoc.py" ]; then
    ./run tool/md_to_gdoc.py --help > /dev/null 2>&1 && pass "md_to_gdoc.py --help works" || echo "  Note: md_to_gdoc.py --help returned non-zero (may be expected)"
fi
