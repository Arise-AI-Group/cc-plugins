...
if command -v python3 &> /dev/null; then
    ...
    [ "$name" = "ssh" ] && pass "Plugin name correct: $name" || fail "Plugin name incorrect: $name"
fi
...
if [ -f "tool/ssh_client.py" ]; then
    ./run tool/ssh_client.py --help > /dev/null 2>&1 && pass "ssh_client.py --help works" || echo "  Note: ssh_client.py --help returned non-zero (may be expected)"
fi
