#!/bin/bash
# Test script for demo-deploy plugin
# Usage: ./test.sh
...
echo "Testing demo-deploy plugin..."
echo ""
...
if command -v python3 &> /dev/null; then
    ...
    [ "$name" = "demo-deploy" ] && pass "Plugin name correct: $name" || fail "Plugin name incorrect: $name"
fi
...
if [ -f "tool/demo_deploy.py" ]; then
    ./run tool/demo_deploy.py --help > /dev/null 2>&1 && pass "demo_deploy.py --help works" || echo "  Note: demo_deploy.py --help returned non-zero (may be expected)"
fi
