#!/bin/bash
# Test script for diagrams plugin
# Usage: ./test.sh
...
if command -v python3 &> /dev/null; then
    ...
    [ "$name" = "diagrams" ] && pass "Plugin name correct: $name" || fail "Plugin name incorrect: $name"
fi
...
if [ -f "tool/generate_drawio.py" ]; then
    ./run tool/generate_drawio.py --help > /dev/null 2>&1 && pass "generate_drawio.py --help works" || echo "  Note: generate_drawio.py --help returned non-zero (may be expected)"
fi
if [ -f "tool/migrate_ascii_to_mermaid.py" ]; then
    ./run tool/migrate_ascii_to_mermaid.py --help > /dev/null 2>&1 && pass "migrate_ascii_to_mermaid.py --help works" || echo "  Note: migrate_ascii_to_mermaid.py --help returned non-zero (may be expected)"
fi
if [ -f "tool/generate_mermaid.py" ]; then
    ./run tool/generate_mermaid.py --help > /dev/null 2>&1 && pass "generate_mermaid.py --help works" || echo "  Note: generate_mermaid.py --help returned non-zero (may be expected)"
fi
